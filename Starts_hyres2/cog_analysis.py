...
def compute_full_interval(boat1_df: pd.DataFrame, boat1_name: str = "boat1") -> list[dict]:
    start = boat1_df['SecondsSince1970'].min()
    end = boat1_df['SecondsSince1970'].max()
    duration = end - start
    ...
    interval = {
        ...
        'duration': duration,
        'boat1_name': boat1_name,
        'avg_SOG_boat1': boat1_df['SOG'].mean(),
        'SOG_variation_boat1': boat1_df['SOG'].std(),
    }
...
def analyze_session(boat1_path: str, smoothing_window_sec: float = 20.0, min_duration_sec: float = 20.0, sog_derivative_threshold: float = 0.2) -> list[dict]:
    ...
    boat1_df['SOG_derivative'] = (
            boat1_df['SOG_smoothed'].diff(2) / (2.0 / freq1)
        ).abs()
    min_freq = freq1
    ...
    # )
    longest_intervals = compute_full_interval(boat1_df, boat1_name)
