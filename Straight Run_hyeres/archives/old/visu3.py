...
def process_run(df1, df2, name1, name2):
    ...
    if df1.empty or df2.empty:
        print(f"⚠️ Skipping after time alignment: {name1} vs {name2}")
        return

    print(f"✔️ Processing: {name1} ({len(df1)}) vs {name2} ({len(df2)})")
...
def process_all_run(run_path, summary_path):
    ...
    print(f"→ Full interval: {name1} vs {name2}")
    process_run(result["full_df1"], result["full_df2"], name1, name2)

    print(f"→ Interval 1: {name1} vs {name2}")
    process_run(result["reduced_boat1_int1_df"], result["reduced_boat2_int1_df"], name1, name2)

    print(f"→ Interval 2: {name1} vs {name2}")
