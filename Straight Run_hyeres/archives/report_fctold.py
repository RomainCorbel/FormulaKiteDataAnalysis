...
def compute_directional_gain(df1, df2):
    ...
    try:
        ...
        print(f"Error computing directional gain: {e}", flush=True)
        return _empty_result()
...
def process_run(df1, df2, name1, name2, title):
    if df1.empty or df2.empty:
        print(f"⚠️ Skipping {title} due to empty data: {name1} vs {name2}", flush=True)
        return
...
def process_all_run(run_path, summary_path, tot=False, onlyUpwind=False, onlyDownwind=False):
    ...
    run_name = os.path.basename(run_path)
    print("\n\n\n\n" + "=" * 80, flush=True)
    print(f"Processing run: {run_name}", flush=True)
    print("=" * 80 + "\n", flush=True)
    ...
    if onlyDownwind or (not onlyUpwind and not onlyDownwind):
        print("-" * 40, flush=True)
        print("Interval 2: Downwind", flush=True)
        print("-" * 40, flush=True)
