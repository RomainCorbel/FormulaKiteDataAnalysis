...
def compute_longest_intervals(
    ...
    window_boat2 = int(smoothing_window_sec * freq_boat2)
    ...
    boat2_df['SOG_smoothed'] = boat2_df['SOG'].rolling(
        window=window_boat2,
        center=True,
        min_periods=1
    ).mean()
    ...
    boat2_df['SOG_derivative'] = (
        boat2_df['SOG_smoothed'].diff(2) / (2.0 / freq_boat2)
    ).abs()
    intervals = []
...
def add_avg_twa_to_intervals(intervals: list[dict], boat1_df: pd.DataFrame, boat2_df: pd.DataFrame) -> list[dict]:
    ...
    for interval in intervals:
        ...
        # Moyenne TWS pour boat2
        mask2 = (boat2_df['SecondsSince1970'] >= start) & (boat2_df['SecondsSince1970'] <= end)
        tws_values2 = boat2_df.loc[mask2, 'TWS']
        interval['avg TWS boat2'] = tws_values2.mean() if not tws_values2.empty else None
...
def analyze_session(boat1_path: str, boat2_path: str, smoothing_window_sec: float = 20.0, min_duration_sec: float = 20.0, sog_derivative_threshold: float = 0.2) -> list[dict]:
    """
